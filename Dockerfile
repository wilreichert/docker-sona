...
RUN echo debconf shared/accepted-oracle-license-v1-1 select true | debconf-set-selections && \
    ...
    apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys EEA14886 && \
    apt-get update && \
    apt-get install -y less zip curl oracle-java8-installer oracle-java8-set-default
...
COPY --from=build /root/onos/buck-out/gen/tools/package/onos-package/onos.tar.gz \
      /root/onos.tar.gz
COPY --from=build /root/onos/apps/openstacknetworking/target/onos-apps-openstacknetworking-1.11.0-SNAPSHOT.jar \
      /root/onos-apps-openstacknetworking-1.11.0-SNAPSHOT.jar

RUN     cd /root && \
        mkdir onos-service && \
        tar -xf onos.tar.gz -C onos-service --strip-components=1 && \
        rm onos.tar.gz && \
        cd onos-service && \
        ...
        rm *.jar && \
        mv /root/onos-apps-openstacknetworking-1.11.0-SNAPSHOT.jar onos-apps-openstacknetworking-1.11.0-SNAPSHOT.jar
